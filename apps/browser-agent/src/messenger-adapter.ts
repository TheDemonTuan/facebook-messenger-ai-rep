...
export class PlaywrightMessengerAdapter implements ChannelAdapter {
  ...
  private async processInboundBubbles(
    ...
    if (lastOutgoingIdx >= 0 && this.externalOutboundCallback) {
      ...
      if (lastOutBubble) {
        ...
        if (!isThreadFirstBaseline && !this.seenOutgoingBubbleIds.has(outBubbleId)) {
          ...
          // Check durable bot checker for outgoing bubble:
          // CRITICAL: For outgoing bubbles, ONLY an exact message reference (EXACT_EXTERNAL_REF) proves it is a bot message!
          // Text matching (STRICT_TEXT_MATCH) must NEVER prevent human takeover detection, because a human operator
          // on a mobile phone might send the same phrase (e.g. "Dạ còn hàng nha") as a previous bot message.
          ...
          if (this.isDurableBotOutboundChecker) {
            try {
              ...
              isDurableBot = res === "EXACT_EXTERNAL_REF";
            } catch {
              // Ignore
            }
          }
          ...
          if (isBot) {
          ...
          } else if (outText.length > 0 || hasMedia) {
            console.log(
            ...
            try {
              ...
              // CRITICAL: Only commit to seenOutgoingBubbleIds AFTER callback successfully executes!
              // If callback throws (DB/network error), bubble remains uncommitted so next poll can retry takeover.
              this.seenOutgoingBubbleIds.add(outBubbleId);
              if (this.seenOutgoingBubbleIds.size > 1000) {
                const firstKey = this.seenOutgoingBubbleIds.values().next().value;
                if (firstKey) this.seenOutgoingBubbleIds.delete(firstKey);
              ...
              console.error("[BrowserAdapter] Error in externalOutboundCallback (will retry on next poll):", err);
            }
          }
        }
      }
    ...
    for (let i = startIdx; i < bubbleResult.bubbles.length; i++) {
      ...
      // CRITICAL: Text matching alone must NEVER suppress an incoming bubble (!bubble.isOutgoing)!
      // Whether customer identity is verified or unverified, a customer saying "ok" or repeating bot text
      // must NOT be dropped as a bot echo. Only an EXACT external message reference (confirmed bot ID)
      // proves that the bubble was generated by the bot.
      ...
      const shouldSuppressAsBotEcho = durableBotEvidence === "EXACT_EXTERNAL_REF";
    ...
    try {
      ...
      while (Date.now() - startTime < timeoutMs) {
        ...
        for (let index = bubbles.length - 1; index >= 0; index--) {
          ...
          // CRITICAL: An outgoing bot delivery verification MUST require the bubble to be outgoing (b.isOutgoing === true)!
          // Incoming bubbles (from customers, unverified participants, or system) must NEVER be used to confirm bot delivery,
          // even if the text matches exactly.
