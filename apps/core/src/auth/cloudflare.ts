...
export async function verifyCloudflareAccess(
  ...
  if (isProduction) {
    ...
    if (!env.CLOUDFLARE_ACCESS_AUD || !env.CLOUDFLARE_ACCESS_TEAM_NAME) {
      reply.status(500).send({ error: "Cloudflare Access AUD and Team Name must be configured in production" });
      return false;
    }
    ...
    try {
      ...
      if (!email) {
        reply.status(403).send({ error: "Cloudflare Access JWT assertion missing email/sub claim" });
        return false;
      }
